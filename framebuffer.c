...
void ClearChar(UINT16 x, UINT16 y, UINT32 color)
{   
	UINT16 i, j;
    for (i = x *8; i < x * 8 + 8; i++)
		for (j = y * 8; j < y*8+8; j++)
...
void ClearString(UINT16 x, UINT16 y, UINT16 length, UINT32 color)
{
	UINT16 i = 0;
	
	for (i = 0; i < length; i++)
		ClearChar(x+i, y, color);
}
...
void Scroll()
{
	UINT16 *addr1, *addr2;
	UINT16 row,col;
	for(col=0;col<ysize-16;col++)
		for(row=0;row<xsize;row++) {	
			addr1 = (UINT16 *)fb_base_addr + (col * xsize + row);
			addr2 = (UINT16 *)fb_base_addr + ((col+8) * xsize + row);
			*addr1 = *addr2;
		}
}
